...
def generate_profiling_report(records: List[ImageRecord], output_dir: Path):
    ...
    for r in records:
        data.append({
            ...
            "ranking_reasons": " | ".join(r.ranking_reasons) if r.ranking_reasons else "",
            "is_best_frame": r.is_best_frame,
            "classification": r.classification,
            "classification_score": r.classification_score,
            "confidence_score": r.confidence_score,
            "processing_time_ms": r.processing_time_ms,
            "classification_reasons": " | ".join(r.classification_reasons) if r.classification_reasons else "",
    ...
    csv_columns = ["filename", "blur_score", "subject_sharpness", "face_sharpness", "eye_sharpness", "luminance_noise", "subject_visibility_score", "duplicate_group_id", "similarity_score", "perceptual_hash"]
    ...
    df[["filename", "duplicate_group_id", "perceptual_hash", "similarity_score"]].to_csv(duplicate_groups_path, index=False)
    logger.info(f"Saved duplicate groups to {duplicate_groups_path}")
    
    burst_ranking_path = output_dir / "burst_ranking.csv"
    df[["duplicate_group_id", "filename", "burst_rank_position", "burst_rank_score", "ranking_reasons", "is_best_frame"]].to_csv(burst_ranking_path, index=False)
    logger.info(f"Saved burst ranking to {burst_ranking_path}")
    
    # Shared logic for Classification / Validation Outputs
    classification_cols = [
        "filename", "classification", "classification_score", "confidence_score",
        "editability_score", "editability_confidence", "shadow_recovery_score", 
        "highlight_recovery_score", "recovery_noise", "recovery_failure_reason", 
        "processing_time_ms", "classification_reasons", "duplicate_group_id", 
        "burst_rank_position", "is_best_frame", "eye_state_score", "expression_score", 
        "expression_type", "expression_confidence", "face_visibility_score", 
        "face_quality_score", "number_of_faces", "primary_face_index", "largest_face_ratio"
    ]
    ...
    metrics_to_plot = [
        "global_sharpness", "subject_sharpness", "face_sharpness", 
        "luminance_noise", "subject_visibility_score", "underexposure_score"
    ]
