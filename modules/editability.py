...
def editability_analysis_module() -> Callable[[ImageRecord], ImageRecord]:
    def analyze_editability(record: ImageRecord) -> ImageRecord:
        ...
        try:
            ...
            if img_bgr is None:
                logger.error(f"Module 'EditabilityEngine' failed on '{record.filename}'. Reason: Image could not be loaded. Action: Skipping editability analysis.")
                return record
            ...
            if shadow_ratio > 0.02: # Only penalize if more than 2% of image is deep shadows
            ...
            logger.debug(f"Editability {record.filename}: Score={record.editability_score:.1f}, " 
                         f"Conf={record.editability_confidence:.1f}, Reason={record.recovery_failure_reason}")
            
        except Exception as e:
            logger.error(f"Module 'EditabilityEngine' failed on '{record.filename}'. Reason: {str(e)}. Action: Skipping editability analysis.")
