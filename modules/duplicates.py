...
def group_duplicates(records: List[ImageRecord], backend: DuplicateDetectionBackend = None, threshold: float = 10.0, time_window_seconds: float = 10.0) -> List[ImageRecord]:
    """
    ...
    for record in records:
        ...
        if anchor_record and anchor_record.perceptual_hash:
            time_diff = 0.0
            if record.exif and record.exif.capture_time and anchor_record.exif and anchor_record.exif.capture_time:
                time_diff = abs((record.exif.capture_time - anchor_record.exif.capture_time).total_seconds())
                
            distance = backend.calculate_distance(record.perceptual_hash, anchor_record.perceptual_hash)
    ...
    logger.info(f"Duplicate detection complete: Found {group_count} unique groups covering {duplicate_count} duplicates.")
    return records
...
def rank_burst_groups(records: List[ImageRecord]) -> List[ImageRecord]:
    ...
    for _, group_records in groups.items():
        ...
        for r in group_records:
            ...
            if ss > 100: reasons.append("High Subject Sharpness")
            
            # 2. Subject Visibility
            sv = r.subject_visibility_score or 0.0
            score += sv * 100 * 0.3
            if sv > 0.7: reasons.append("Good Subject Visibility")
            
            # 3. Face Sharpness
            fs = r.face_sharpness or 0.0
            score += min(fs, 500) * 0.2
            if fs > 100: reasons.append("High Face Sharpness")
            
            # 4. Face Confidence
            fc = r.face_confidence or 0.0
            score += fc * 100 * 0.1
            if fc > 0.6: reasons.append("Confident Face Detection")
            
            # 5. Noise Penalty
            ln = r.luminance_noise or 0.0
            score -= ln * 5.0
            if ln > 4.0: reasons.append("High Noise Penalty")
            
            # 6. Exposure Penalty (Weak)
            ue = r.underexposure_score or 0.0
            score -= min(ue, 50) * 0.5
            if ue > 30: reasons.append("Underexposure Penalty")
