...
def generate_visual_exports(records: List[ImageRecord], output_dir: Path, max_samples: int = None):
    """
    ...
    grouped["REJECT"].sort(key=lambda x: x.classification_score or 0.0, reverse=False) # Lowest first
    ...
    def build_sheet(records_list, title, filename, color):
        ...
        avg_score = sum(r.classification_score or 0.0 for r in records_list) / len(records_list)
        ...
        for idx, r in enumerate(records_list):
            ...
            # Draw text
            text_y = y + thumb_h + 10
            draw.text((x + 10, text_y), f"{r.filename}", fill=(255, 255, 255), font=font)
            draw.text((x + 10, text_y + 20), f"{r.classification} | Score: {r.classification_score:.1f}", fill=(200, 200, 200), font=font)
            
        sheet_img.save(sheets_dir / filename, quality=85)
        
    build_sheet(grouped["KEEP"], "KEEP Classification", "keep_sheet.jpg", (50, 205, 50))
    build_sheet(grouped["REVIEW"], "REVIEW Classification", "review_sheet.jpg", (255, 215, 0))
    build_sheet(grouped["REJECT"], "REJECT Classification", "reject_sheet.jpg", (220, 20, 60))
    
    # Top KEEP
    if len(grouped["KEEP"]) > 0:
        build_sheet(grouped["KEEP"][:20], "Top KEEP Showcase", "top_keep_sheet.jpg", (50, 205, 50))
