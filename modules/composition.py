...
def process_composition(record: ImageRecord) -> ImageRecord:
    ...
    try:
        ...
        if img is None:
            logger.error(f"Module 'CompositionAnalysis' failed on '{record.filename}'. Reason: Image could not be loaded. Action: Skipping composition analysis.")
            return record
        ...
        logger.error(f"Module 'CompositionAnalysis' failed on '{record.filename}'. Reason: {str(e)}. Action: Skipping composition analysis.")
