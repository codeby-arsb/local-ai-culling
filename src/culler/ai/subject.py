...
def process_subject(record: ImageRecord) -> ImageRecord:
    ...
    try:
        ...
        if len(boxes) > 0:
            ...
            for box in boxes:
                ...
                if area_pct > best_area_percentage:
                    ...
                    if gray_img is not None:
                        ...
                        if roi.size > 0:
                            record.subject_sharpness = float(
            ...
            record.subject_visibility_score = calculate_visibility(
                best_area_percentage, best_confidence
            )
        else:
        ...
        logger.error(
            f"Module 'SubjectAnalysis' failed on '{record.filename}'. Reason: {str(e)}. Action: Skipping subject analysis."
        )
