...
def calculate_confidence(
    classification: str, score: float, is_burst_safety_net: bool
) -> float:
...
def classify_records(records: List[ImageRecord]) -> List[ImageRecord]:
    ...
    for r in records:
        ...
        if AppConfig.get("modules.expression_intelligence", True):
            ...
            dominant_subject = (
                r.subject_count == 1 and (r.subject_area_percentage or 0.0) > 10.0
            )
            large_face = (r.largest_face_ratio or 0.0) > 0.02
            if (
                dominant_subject
                and large_face
                and r.eye_state in ["Both Eyes Closed", "Mid Blink"]
            ):
        ...
        if ss < 25:
            reasons.append("Low Subject Sharpness")
        if sv > 0.6:
            reasons.append("Strong Subject Visibility")
        if sv < 0.2:
            reasons.append("Poor Subject Visibility")
        if ln > 4.0:
            reasons.append("Elevated Noise")
        if r.is_best_frame:
            reasons.append("Strongest Frame in Burst (or Standalone)")
        else:
            reasons.append("Non-Best Frame in Burst")
        ...
        # 4. Confidence Engine
        r.confidence_score = calculate_confidence(
            classification, score, is_burst_safety_net
        )

        counts[classification] += 1

    logger.info(
        f"Classification complete. KEEP: {counts['KEEP']}, REVIEW: {counts['REVIEW']}, REJECT: {counts['REJECT']}"
    )
    return records
