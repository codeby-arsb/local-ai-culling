...
def process_blur(record: ImageRecord) -> ImageRecord:
    ...
    try:
        ...
        if img is None:
            logger.error(
                f"Module 'BlurAnalysis' failed on '{record.filename}'. Reason: Image could not be loaded. Action: Skipping blur analysis."
            )
            return record
        ...
        logger.error(
            f"Module 'BlurAnalysis' failed on '{record.filename}'. Reason: {str(e)}. Action: Skipping blur analysis."
        )
