...
def update_summary():
    ...
    avg_review_time = (
        df_judged["review_time_ms"].mean()
        if "review_time_ms" in df_judged.columns
        else 0.0
    )
    agreed_rejects = len(
        df_judged[
            (df_judged["ai_classification"] == "REJECT")
            & (df_judged["photographer_decision"] == "REJECT")
        ]
    )
...
def get_inspector_metadata(filename: str):
    ...
    if not metadata_json.exists():
        return JSONResponse(
            {"error": "Metadata not found for this image"}, status_code=404
        )
...
def manage_session(output_dir: Path):
    ...
    if needs_archive:
        ...
        logger.info(
            f"New dataset detected. Archiving old session: {dataset_name} ({old_id})"
        )
...
def run_server(output_dir: Path, port: int = 8000):
    ...
    for folder in ["keep_preview", "review_preview", "reject_preview"]:
        ...
        if folder_path.exists():
            app.mount(
                f"/{folder}", StaticFiles(directory=str(folder_path)), name=folder
            )
        else:
