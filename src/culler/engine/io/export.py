        ...
        for rank_idx, r in enumerate(cl_records):
            ...
            if mode == "hardlink":
                try:
                    ...
                    logger.error(
                        f"Failed to create hardlink for {r.original_path}: {e}"
                    )
            else:
                logger.warning(
                    f"Unsupported export mode '{mode}'. Skipping {r.filename}."
                )
            ...
            csv_data.append(
                {
                    ...
                    "Filename": r.filename,
                    "Decision": cls,
                    "Classification Score": round(r.classification_score, 2)
                    if r.classification_score is not None
                    else "",
                    "Confidence": round(r.confidence_score, 2)
                    if r.confidence_score is not None
                    else "",
                    "Editability": round(r.editability_score, 2)
                    if r.editability_score is not None
                    else "",
                    "Expression": r.expression_type or "",
                    "Eye State": r.eye_state or "",
                    "Burst Rank": r.burst_rank_position
                    if r.burst_rank_position is not None
                    else "",
