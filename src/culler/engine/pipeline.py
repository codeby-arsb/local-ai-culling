    ...
    def process(self, image_record: ImageRecord) -> ImageRecord:
        ...
        if image_record.preview_path:
            try:
                ...
                logger.error(
                    f"Failed to load cache image {image_record.preview_path}: {e}"
                )
        ...
        for module in self.modules:
            module_name = (
                ...
                else str(module.__class__.__name__)
            )
            try:
                ...
                logger.error(
                    f"Module '{module_name}' failed on '{image_record.filename}'. Reason: {e}"
                )
                logger.debug(traceback.format_exc())
                image_record.classification = ClassificationType.REVIEW
                image_record.classification_reasons.append(
                    f"Pipeline error in {module_name}: {str(e)}"
                )
